...
export async function createUserDocument(uid, data) {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      darkMode:false, language:'th', fcmToken:'',
      // PDPA consent fields
...
export const listenUser         = (uid, cb, errCb) => onSnapshot(doc(db,'users',uid), s => cb(s.exists()?{id:s.id,...s.data()}:null), err => { if (errCb) errCb(err); else console.warn('[listenUser]', err.message); })
...
export const listenUserCars = (uid, cb, errCb) => onSnapshot(query(collection(db,'cars'), where('userId','==',uid), orderBy('createdAt')), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))), err => { if (errCb) errCb(err); else console.warn('[listenUserCars]', err.message); })
...
// Helper sync ข้อมูลการบำรุงรักษา/เลขไมล์ไปยัง cars doc ของลูกค้า
export async function syncCarMaintenance({ carId, plate, userId, currentMileage, nextServiceMileage, nextServiceDate, serviceType }) {
  try {
    let targetCarId = carId
    if (!targetCarId && (plate || userId)) {
      const q = userId
        ? query(collection(db, 'cars'), where('userId', '==', userId))
        : query(collection(db, 'cars'), where('plate', '==', plate))
      const snap = await getDocs(q)
      const found = snap.docs.find(d => !plate || d.data().plate === plate) || snap.docs[0]
      if (found) targetCarId = found.id
    }
    if (!targetCarId) return false

    const updatePayload = {
      updatedAt: serverTimestamp(),
    }
    if (currentMileage !== undefined && currentMileage !== null && currentMileage !== '') {
      updatePayload.currentMileage = Number(currentMileage)
      updatePayload.lastServiceMileage = Number(currentMileage)
    }
    if (nextServiceMileage !== undefined && nextServiceMileage !== null && nextServiceMileage !== '') {
      updatePayload.nextServiceMileage = Number(nextServiceMileage)
    }
    if (nextServiceDate) {
      updatePayload.nextServiceDate = nextServiceDate
    }
    if (serviceType) {
      updatePayload.lastServiceType = serviceType
    }
    updatePayload.lastServiceDate = serverTimestamp()

    await updateDoc(doc(db, 'cars', targetCarId), updatePayload)
    return true
  } catch (e) {
    console.warn('[syncCarMaintenance]', e.message)
    return false
  }
}
...
export const listenUserBookings  = (uid, cb, errCb) => onSnapshot(query(collection(db,'bookings'),where('userId','==',uid),orderBy('createdAt','desc')), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))), err => { if (errCb) errCb(err); else console.warn('[listenUserBookings]', err.message); })

// ─── Repairs
// ฟัง repair ที่กำลังดำเนินการอยู่ของลูกค้า (ใช้แสดงสถานะบนหน้า /status)
export const listenActiveRepairByUser = (uid, cb, errCb) => {
  const activeStatuses = ['waiting', 'diagnosing', 'awaiting_approval', 'repairing', 'qc']
  return onSnapshot(
    ...
    (s) => {
      ...
      cb(activeDocs.length > 0 ? activeDocs[0] : null)
    },
    (err) => {
      if (errCb) errCb(err)
      else console.warn('[listenActiveRepairByUser]', err.message)
    }
  )
}
...
// ─── Notifications
export const listenNotifications    = (uid, cb, errCb) => onSnapshot(query(collection(db,'notifications'), where('userId','==',uid), orderBy('createdAt','desc'), limit(30)), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))), err => { if (errCb) errCb(err); else console.warn('[listenNotifications]', err.message); })
...
export const createArticle = (data) => addDoc(collection(db,'articles'), {...data, reads:0, deleted:false, createdAt:serverTimestamp(), updatedAt:serverTimestamp()})
...
// Staff: get all articles (including unpublished, but excluding deleted)
export async function getAllArticles() {
  const s = await getDocs(query(collection(db,'articles'), orderBy('createdAt','desc')))
  return s.docs.filter(d => d.data().deleted !== true).map(d=>({id:d.id,...d.data()}))
}
